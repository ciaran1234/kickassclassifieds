...
var validateAccountToken = function (key, token) {
    return client.hgetallAsync(key + ':' + token)
        .then(hash => {
            if (!hash) throw new InvalidAccountTokenError('invalid token provided');
            ...
            if (!validExpiration || hashSecret(config.accountConfirmationSecret, hash.salt) !== token) throw new InvalidAccountTokenError('invalid token provided');
...
exports.signup = function (registration) {
    ...
    return user.save() // should throw an entity validation error.... check if we can pipe that into mongoose
...
exports.signin = function (model) {
    return User.findOne({ email: model.email })
        .then(user => {
            if (!(user && user.authenticate(model.password))) throw new Error('User not authenticated');
...
exports.update = function (user) {
    ...
    return user.save()
        .then(user => { return user; })
        .catch(error => { throw new EntityValidationError('user update failed', error); });
};
